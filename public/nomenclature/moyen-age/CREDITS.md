# Crédits photos — nomenclature Moyen Âge

Source : **Wikimedia Commons** (licences libres — voir chaque page fichier).

| Fichier | Sujet | Source Commons |
|---------|-------|----------------|
| `chateau.jpg` | Château | Chateau_de_Chenonceau_2008.jpg |
| `chevalier.jpg` | Armure / chevalier | Warwick_Castle_knight+horse_1.jpg |
| `dragon.jpg` | dragon (créature) | Dragon statue.jpg |
| `couronne.jpg` | Couronne | St_Edward's_Crown.jpg |
| `blason.jpg` | Blason / écu | Dish with Heraldic Shield MET sf14-40-728d1.jpg |
| `epee.jpg` | Épée | Francois_I_sword-J_316-IMG_1805-white.jpg |

Retélécharger : `node scripts/fetch-nomenclature-photos.mjs`
| `bouclier.jpg` | bouclier | Shield.jpg |
| `casque.jpg` | casque | Medieval helmet.jpg |
| `tour.jpg` | tour | Castle tower.jpg |
| `cheval.jpg` | cheval | Horse.jpg |
| `trone.jpg` | trone | Throne.jpg |
| `pont-levis.jpg` | pont-levis | Pont-levis.jpg |
