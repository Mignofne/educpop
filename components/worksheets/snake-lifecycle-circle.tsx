...
import { PictoEgg, PictoScale, PictoSnake } from "./art/pictos-animaux"

/** Cycle serpent 4–5 — ronde + étiquettes à coller */
const STEPS = [
  { id: "oeuf", label: "l'œuf", angle: -90 },
  { id: "bebe", label: "le bébé serpent", angle: 0 },
  { id: "mue", label: "la mue", angle: 90 },
  { id: "adulte", label: "le serpent adulte", angle: 180 },
] as const

const LABELS_SHUFFLED = ["la mue", "l'œuf", "le serpent adulte", "le bébé serpent"]

export function SnakeLifecycleCircle() {
  const cx = 160
  const cy = 155
  const r = 100
    ...
    >
      <div className="mx-auto max-w-lg">
        <svg
          ...
          aria-label="Cycle de vie du serpent en cercle"
        >
          <circle
            cx={cx}
            cy={cy}
          ...
          <defs>
            <marker id="sn-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
              <path d="M0 0 L6 3 L0 6 Z" fill="currentColor" />
            </marker>
          </defs>

          <circle cx={cx} cy={cy} r="28" className="fill-leaf" stroke="currentColor" strokeWidth="3" opacity="0.45" />
          <text
            x={cx}
            y={cy + 5}
            textAnchor="middle"
...
function stepArt(id: string) {
  switch (id) {
    ...
    case "bebe":
      return <PictoSnake mode="color" width={80} height={80} />
    case "mue":
      return <PictoScale mode="color" width={80} height={80} />
    case "adulte":
      return <PictoSnake mode="color" width={80} height={80} />
    default:
      return null
